...
int capture_start()
{
    ...
    for (int vfb = 0; vfb < vfbcap.numVfbs; vfb++) {
        ...
        // TODO: probably supports only one vfb, whatever this means
        // mmap was failing due to bad offset
        // so we map the whole thing once
        size_t fix_ptr = (size_t) mmap(0, vfbprop.stride * vfbprop.height * vfbcap.numPlanes, PROT_READ, MAP_SHARED, mem_fd, vfbprop.ptr[vfb][0]);
        for (int plane = 0; plane < vfbcap.numPlanes; plane++) {
            DBG("[DILE_VT] vfb[%d][%d] = 0x%08x ", vfb, plane, vfbprop.ptr[vfb][plane]);
            size_t start_address = (size_t)vfbprop.ptr[vfb][0];
            size_t current_address = (size_t)vfbprop.ptr[vfb][plane];
            vfbs[vfb][plane] = (uint8_t *)(fix_ptr + (current_address - start_address));
...
//    usleep(33000);
    ...
    const uint64_t delay = 1000000/17;
    uint64_t now_time = getticks_us();
    if(now_time < prev_time + delay){
        usleep(prev_time+delay - now_time);
    }
    prev_time = now_time;
